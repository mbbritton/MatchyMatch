...
import { buildGabbyDeck, pickRandomGabbySet } from '../../data/gabbyGiftData'

function initializeGame() {
  const set = pickRandomGabbySet()
  return {
    set,
    cards: buildGabbyDeck(set),
    flipped: new Set(),
    matched: new Set(),
    moves: 0,
  ...
  return (
    <button
      ...
      style={{
        ...
        background: isMatched ? 'rgba(94, 92, 230, 0.12)' : 'var(--bg-surface)',
        ...
        perspective: '1000px',
        boxShadow: isMatched
          ? 'inset 0 2px 8px rgba(94, 92, 230, 0.2)'
          : 'var(--shadow-sm)',
...
export default function GabbysGiftBoard() {
...
function Game({ onNewGame }) {
  ...
  const { set, cards, flipped, matched, moves, gameState } = state
  const totalPairs = set.cards.length
        ...
        >
          <div
            style={{
              ...
              height: 72,
              borderRadius: 20,
              background: 'linear-gradient(145deg, #5e5ce6, #0086ea)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              fontSize: 32,
              boxShadow: '0 8px 24px rgba(94, 92, 230, 0.35)',
            }}
          >
            🎁
          </div>
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full max-w-lg mx-auto">
      ...
      <div className="w-full flex flex-col items-center gap-2">
        ...
        >
          {set.emoji} {set.label} — match every pair!
        </p>
      </div>
      ...
      >
        Tap cards to flip them and find every matching pair!
      </p>
